...
import { Select, MultiSelect, TextInput, Button, Paper, Title, Group, Textarea } from '@mantine/core';
...
const QueryBuilder = () => {
    ...
    useEffect(() => {
        if (selectedTable) {
            axios.get(`http://localhost:8800/columns/${selectedTable}`)
                .then(response => {
    ...
    const handleExecuteQuery = () => {
        const query = `SELECT ${selectedColumns.join(', ')} FROM ${selectedTable} ${conditions ? `WHERE ${conditions}` : ''}`;
        axios.post('http://localhost:8800/query', { query })
            .then(response => {
